 ...
 */
class PressBook_Masonry_Blogs_Scripts extends PressBook\Scripts {
	/**
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_style( 'pressbook-masonry-blogs-fonts', static::fonts_url(), array(), null ); // phpcs:ignore WordPress.WP.EnqueuedResourceParameters.MissingVersion
		...
		wp_enqueue_style( 'pressbook-masonry-blogs-style', get_stylesheet_directory_uri() . '/style.min.css', array(), PRESSBOOK_MASONRY_BLOGS_VERSION );
		wp_style_add_data( 'pressbook-masonry-blogs-style', 'rtl', 'replace' );

		// Add output of customizer settings as inline style.
		wp_add_inline_style( 'pressbook-masonry-blogs-style', PressBook_Masonry_Blogs_CSSRules::output() );
		...
		wp_enqueue_script( 'pressbook-masonry-blogs-script', get_stylesheet_directory_uri() . '/assets/js/script.min.js', $pressbook_masonry_deps, PRESSBOOK_MASONRY_BLOGS_VERSION, true );

		$pressbook_masonry_options = PressBook_Masonry_Blogs_Options::get_masonry();
		wp_localize_script(
			'pressbook-masonry-blogs-script',
			'pressbookMasonry',
			array(
	...
	public static function fonts_url() {
		...
		$source_serif_pro = _x( 'on', 'Source Serif Pro font: on or off', 'pressbook-masonry-blogs' );
		...
		$philosopher = _x( 'on', 'Philosopher font: on or off', 'pressbook-masonry-blogs' );
		if ( 'off' !== $philosopher ) {
		...
		$fonts_url = apply_filters( 'pressbook_masonry_blogs_fonts_url', $fonts_url );
	...
	public function remove_dynamic_styles() {
		if ( wp_style_is( 'pressbook-masonry-blogs-style', 'enqueued' ) ) {
			wp_style_add_data( 'pressbook-masonry-blogs-style', 'after', '' );
		}
	}
}
